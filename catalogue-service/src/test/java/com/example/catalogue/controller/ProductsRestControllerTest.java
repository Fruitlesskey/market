...
class ProductsRestControllerTest {
  ...
  @InjectMocks ProductsRestController controller;
  ...
  void findProduct_ReturnsProductsList() {
    ...
    doReturn(
            List.of(
                new Product(1, "Первый товар", "Описание первого товара"),
  ...
  void createProduct_RequestIsValid_ReturnsNoContent() throws BindException {
    ...
    doReturn(new Product(1, "Новое название", "Новое описание"))
        .when(this.productService)
        .createProduct("Новое название", "Новое описание");
    ...
    assertEquals(
        URI.create("http://localhost/catalogue-api/products/1"), result.getHeaders().getLocation());
